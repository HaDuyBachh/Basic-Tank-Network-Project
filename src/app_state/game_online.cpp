...
Game_Online::Game_Online(int players_count)
{
    m_level_columns_count = 0;
    m_level_rows_count = 0;
    m_current_level = 0;
...
Game_Online::Game_Online(std::vector<Player *> players, int previous_level)
{
    m_level_columns_count = 0;
    m_level_rows_count = 0;
...
void Game_Online::update(Uint32 dt)
{
    if(dt > 40) return;
    ...
    else
    {
        ...
        //kiểm tra sự va chạm của quả bóng với mức độ
        ...
        for(auto player : m_players)
            for(auto enemy : m_enemies)
            {
                ...
                //kiểm tra độ va chạm của đạn của người chơi với đối thủ
        ...
        //kiểm tra sự va chạm của đạn đánh chặn với người chơi
        for(auto enemy : m_enemies)
            for(auto player : m_players)
                    checkCollisionEnemyBulletsWithPlayer(enemy, player);

        //kiểm tra va chạm người chơi với bonus
        for(auto player : m_players)
            for(auto bonus : m_bonuses)
                checkCollisionPlayerWithBonus(player, bonus);

        //Kiểm tra va chạm của xe tăng với cấp độ
        ...
        //Nếu xe tăng của mình đang được bảo vệ
        if(m_protect_eagle)
        {
...
void Game_Online::eventProcess(SDL_Event *ev)
{
...
void Game_Online::loadLevel(std::string path)
{
...
bool Game_Online::finished() const
{
    return m_finished;
}

AppState* Game_Online::nextState()
{
...
void Game_Online::checkCollisionTankWithLevel(Tank* tank, Uint32 dt)
{
...
void Game_Online::checkCollisionTwoTanks(Tank* tank1, Tank* tank2, Uint32 dt)
{
...
void Game_Online::checkCollisionBulletWithLevel(Bullet* bullet)
{
...
void Game_Online::checkCollisionBulletWithBush(Bullet *bullet)
{
    if(bullet == nullptr) return;
    if(bullet->collide) return;
...
void Game_Online::checkCollisionPlayerBulletsWithEnemy(Player *player, Enemy *enemy)
{
...
void Game_Online::checkCollisionEnemyBulletsWithPlayer(Enemy *enemy, Player *player)
{
...
void Game_Online::checkCollisionTwoBullets(Bullet *bullet1, Bullet *bullet2)
{
...
void Game_Online::checkCollisionPlayerWithBonus(Player *player, Bonus *bonus)
{
...
// void Game_Online::generateEnemy()
// {
...
void Game_Online::generateBonus()
{
